...
export function CourseWizardStep4() {
  ...
  const {
    ...
    resource: "courses",
    pagination: { current: 1, pageSize: 50 }, // dostosuj do potrzeb
  ...
  useEffect(() => {
    // jeżeli nie ma przekazanego ID (np. wejście bezpośrednio), wybierz pierwszy dostępny
  ...
  const {
    ...
    resource: "topics",
    filters: selectedId
      ? [{ field: "course_id", operator: "eq", value: typeof selectedId === "string" ? parseInt(selectedId, 10) : selectedId }]
      : [],
        ...
        <Card className="mb-6 border-green-200 bg-green-50/50">
          ...
          <CardContent className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Lista kursów poniżej. Nowy kurs jest wyróżniony i zaznaczony — po prawej widzisz jego tematy.
            </p>
          </CardContent>
        </Card>
      )}
              ...
              <ScrollArea className="h-full">
                <div className="space-y-2">
                  {topics.map((t: any, i: number) => (
                    <div key={t.id ?? i} className="rounded-xl border p-3">
                      <div className="flex items-start gap-2">
                        <span className="text-sm text-muted-foreground">{Number(t.position ?? i + 1)}.</span>
